...
import { 
  ...
  Play, Pause, RotateCcw, Timer, Menu, Database, CloudOff, Cloud, Shield, Key, Eye, Sun, Moon
} from 'lucide-react';
...
const useThemeColors = () => {
  ...
  return {
    ...
    tableHeader: isDark ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600',
...
const Input = ({ label, type = "text", value, onChange, placeholder, className = "", min, max }) => {
  ...
  return (
    <div className={`mb-4 ${className}`}>
      ...
      <input type={type} value={value === undefined || value === null ? '' : value} onChange={onChange} placeholder={placeholder} min={min} max={max} className={`w-full ${colors.inputBg} border ${colors.inputBorder} rounded-lg px-4 py-3 ${colors.text} text-sm placeholder-slate-500 focus:outline-none focus:border-orange-500 transition-colors`} />
    </div>
  );
};
...
const Button = ({ children, onClick, variant = "primary", className = "", size = "normal", icon: Icon }) => {
  ...
  const variants = { 
    ...
    secondary: `${colors.isDark ? 'bg-slate-800 text-slate-200' : 'bg-white text-slate-700'} hover:opacity-80 border ${colors.border}`, 
...
const Card = ({ children, title, action, className = "" }) => {
  const colors = useThemeColors();
  return (
    <div className={`${colors.bgCardTransparent} border ${colors.border} rounded-xl p-4 md:p-6 shadow-sm transition-colors ${className}`}>
  ...
  return (
    <div className="w-full h-full relative" style={{ height: height }}>
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible">
        {[0, 0.25, 0.5, 0.75, 1].map(p => { const y = height - padding - (p * (height - 2*padding)); return <line key={p} x1="0" y1={y} x2={width} y2={y} stroke={colors.isDark ? "#334155" : "#cbd5e1"} strokeWidth="0.5" strokeDasharray="4" />; })}
        ...
        {showDots && data.map((d, i) => {
          ...
          return (<g key={i} className="group"><circle cx={x} cy={y} r="5" fill={colors.isDark ? "#1e293b" : "#ffffff"} stroke={color} strokeWidth="2" /><text x={x} y={y - 12} textAnchor="middle" fill={color} fontSize="12" fontWeight="bold">{Number(d[dataKey]).toFixed(1)}</text><text x={x} y={height - 5} textAnchor="middle" fill={colors.isDark ? "#64748b" : "#94a3b8"} fontSize="10">{d[labelKey]}</text></g>);
        })}
      </svg>
    </div>
  );
    ...
    <div className="space-y-6 animate-fade-in">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
         {admins.map(admin => {
           ...
           return (
             <div key={admin.id} className={`${colors.bgCard} border ${colors.border} rounded-xl p-6 relative group transition-all duration-300 ${isExpanded ? 'row-span-2 shadow-xl border-orange-500/30' : ''}`}>
         ...
         {admins.length === 0 && <div className={`${colors.textSec} col-span-full text-center py-10`}>Ekli yönetici bulunmuyor.</div>}
       </div>
...
const ScheduleModule = ({ student, curriculum, onUpdateStudent, user }) => {
  ...
  // Arşiv Silme Fonksiyonu
  const handleDeleteArchive = (weekId) => {
    if(window.confirm("Bu arşiv kaydını kalıcı olarak silmek istediğinize emin misiniz?")) {
        const updatedArchives = student.pastWeeks.filter(w => w.id !== weekId);
        onUpdateStudent({ ...student, pastWeeks: updatedArchives });
    }
  };

  const handleResetSchedule = () => { if(window.confirm("Haftalık programı sıfırlamak üzeresiniz. Mevcut program arşive eklenecek ve liste temizlenecek. Onaylıyor musunuz?")) { const newTotalSolved = (student.stats?.totalSolved || 0) + weeklySolvedQuestions; const newMonthlySolved = (student.stats?.monthlySolved || 0) + weeklySolvedQuestions; const archivedWeek = { id: Date.now(), date: new Date().toLocaleDateString('tr-TR'), assignments: assignments, solvedCount: weeklySolvedQuestions }; onUpdateStudent({ ...student, assignments: [], pastWeeks: [archivedWeek, ...(student.pastWeeks || [])], stats: { ...student.stats, totalSolved: newTotalSolved, monthlySolved: newMonthlySolved } }); } };
  ...
  return (
    <div className="space-y-8 animate-fade-in">
          ...
          {DAYS.map(day => { 
            ...
            <div key={day.id} className={`min-w-[280px] md:min-w-0 ${colors.bgCardTransparent} border ${colors.border} rounded-xl flex flex-col h-full snap-center`}>
                ...
                <div className="p-2 space-y-2 flex-1">
                    {dayTasks.map(task => (<div key={task.id} className={`p-3 rounded-lg border text-xs group relative ${task.status === 'completed' ? 'bg-emerald-900/20 border-emerald-900/50 opacity-75' : `${colors.isDark ? 'bg-slate-800' : 'bg-slate-100'} ${colors.border}`}`}><div className="flex justify-between items-start"><span className="font-bold text-orange-400 truncate w-full pr-12">{task.subject}</span><div className={`flex gap-1 absolute right-2 top-2 z-10 ${colors.isDark ? 'bg-slate-800/80' : 'bg-white/80'} p-1 rounded backdrop-blur-sm shadow-sm border ${colors.border}`}>{isAdmin && <button onClick={(e) => {e.stopPropagation(); handleEdit(task)}} className={`text-slate-400 hover:text-blue-400 p-2`}><Edit2 size={12}/></button>}{!isStudent && <button onClick={(e) => requestDelete(e, task.id)} className="text-slate-400 hover:text-red-500 p-2"><Trash2 size={12}/></button>}</div></div><div className={`${colors.text} mt-1 text-sm font-medium line-clamp-2`}>{task.topic}</div><div className="mt-2 flex flex-col gap-1"><div className={`flex justify-between items-center text-xs ${colors.textSec}`}><span className={`${colors.isDark ? 'bg-slate-700/50' : 'bg-slate-200'} px-1.5 py-0.5 rounded`}>{TASK_TYPES.find(t=>t.id === task.type)?.label}</span>{task.type === 'soru' && <span className="font-bold text-orange-400">{task.count} Soru</span>}</div>{task.source && (<div className="flex items-center gap-1 text-xs text-blue-400 mt-0.5"><Book size={10} /><span className="truncate">{task.source}</span></div>)}</div><button onClick={() => toggleTaskStatus(task.id)} className={`mt-2 w-full py-2 rounded flex items-center justify-center gap-1 ${task.status === 'completed' ? 'bg-emerald-600 text-white' : `${colors.isDark ? 'bg-slate-700 text-slate-400 hover:bg-slate-600' : 'bg-slate-200 text-slate-600 hover:bg-slate-300'}`}`}>{task.status === 'completed' ? <Check size={14}/> : 'Tamamla'}</button></div>))}
      ...
      {student.pastWeeks?.length > 0 && (<Card title="Geçmiş Haftalar Arşivi" className="mt-8"><div className="space-y-4">{student.pastWeeks.map(week => (<div key={week.id} className={`${colors.isDark ? 'bg-slate-800' : 'bg-slate-100'} p-4 rounded-lg flex justify-between items-center`}><div><div className={`font-bold ${colors.text} text-sm`}>Hafta: {week.date}</div><div className={`text-xs ${colors.textSec}`}>{week.assignments.length} Görev Planlandı</div></div><div className="text-right flex items-center gap-4"><div className="text-orange-400 font-bold">{week.solvedCount} Soru Çözüldü</div>{isAdmin && (<button onClick={() => handleDeleteArchive(week.id)} className="text-red-500 hover:bg-red-500/10 p-2 rounded transition-colors"><Trash2 size={16}/></button>)}</div></div>))}</div></Card>)}
    </div>
  );
};
...
const GoalsModule = ({ student, onUpdateStudent, user }) => {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex justify-between items-center"><h2 className={`text-2xl font-bold ${colors.text}`}>Haftalık Hedef Takibi</h2>{isAdmin && <Button icon={Plus} onClick={() => setShowModal(true)}>Yeni Hedef Ekle</Button>}</div>
      <div className="flex overflow-x-auto gap-4 pb-4 md:grid md:grid-cols-7 md:overflow-visible">
        {DAYS.map(day => {
            ...
            <div key={day.id} className={`min-w-[200px] md:min-w-0 ${colors.bgCardTransparent} border ${colors.border} rounded-xl flex flex-col h-full snap-center`}>
              ...
              <div className="p-2 space-y-2 flex-1">
                {dailyGoals.map(goal => {
                  ...
                  return (
                    <div key={goal.id} className={`p-3 rounded-lg border text-xs relative group ${isCompleted ? 'bg-emerald-900/20 border-emerald-900/50' : `${colors.isDark ? 'bg-slate-800' : 'bg-slate-100'} ${colors.border}`}`}>
                      ...
                      <div className={`absolute top-2 right-2 flex gap-1 ${colors.isDark ? 'bg-slate-900/50' : 'bg-white/50'} rounded px-1`}>{isAdmin && <button onClick={() => handleEdit(goal)} className="text-slate-500 hover:text-blue-400 p-2"><Edit2 size={12}/></button>}{!isStudent && <button onClick={(e) => requestDelete(e, goal.id)} className="text-slate-500 hover:text-red-500 p-2"><Trash2 size={12}/></button>}</div>
...
const ExamModule = ({ student, onUpdateStudent, user }) => {
  ...
  const getBestScores = () => { 
    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-6">
        <div className={`${colors.bgCard} p-4 rounded-xl border ${colors.border} text-center`}>
        ...
        {subjectKeys.map(sub => (
          <div key={sub.key} className={`${colors.bgCard} p-4 rounded-xl border ${colors.border} text-center`}>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className={`flex flex-col lg:flex-row justify-between items-start lg:items-center ${colors.bgCardTransparent} p-4 rounded-xl border ${colors.border} gap-4`}><div className={`flex flex-wrap gap-2 ${colors.isDark ? 'bg-slate-800' : 'bg-slate-200'} p-1 rounded-lg`}>{['ALL', 'TYT', 'AYT', 'LGS', 'BRANS'].map(type => (<button key={type} onClick={() => setFilterType(type)} className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${filterType === type ? 'bg-orange-600 text-white shadow' : `${colors.textSec} hover:${colors.text}`}`}>{type === 'ALL' ? 'Genel' : type === 'BRANS' ? 'Branş' : type}</button>))}</div>{filterType === 'BRANS' && (<select value={branchFilter} onChange={(e) => setBranchFilter(e.target.value)} className={`${colors.inputBg} border ${colors.inputBorder} ${colors.text} text-sm rounded-lg p-2.5 outline-none focus:border-orange-500`}>{BRANCH_SUBJECTS.map(sub => <option key={sub.id} value={sub.id}>{sub.label}</option>)}</select>)}<div className="flex gap-2 ml-auto"><Button size="small" variant={view === 'list' ? 'primary' : 'ghost'} onClick={() => setView('list')}>Liste</Button>{isAdmin && <Button size="small" variant={view === 'add' ? 'primary' : 'ghost'} icon={Plus} onClick={() => setView('add')}>Ekle</Button>}</div></div>
      {view === 'list' && (<><Card title={`${filterType === 'BRANS' ? BRANCH_SUBJECTS.find(b=>b.id===branchFilter)?.label + ' Net Gelişimi' : 'Genel Net İlerleme Analizi'}`}><div className="h-64"><LineChart data={chartData} dataKey="totalNet" labelKey="date" /></div></Card>{getBestScores()}<Card title="Detaylı Sınav Arşivi"><div className="overflow-x-auto"><table className="w-full text-left text-sm whitespace-nowrap"><thead className={`${colors.tableHeader} text-xs uppercase font-bold`}><tr>{getTableHeaders().map((h, i) => <th key={i} className="p-4 text-center">{h}</th>)}</tr></thead><tbody className={`divide-y ${colors.divider} ${colors.isDark ? 'text-slate-300' : 'text-slate-700'}`}>{filteredExams.map(exam => (<tr key={exam.id} className={`${colors.hoverBg} group transition-colors`}><td className={`p-4 ${colors.textSec} text-center`}>{exam.date}</td><td className={`p-4 font-medium ${colors.text} text-center`}>{exam.name}</td>{(filterType === 'ALL' || filterType === 'BRANS') ? (<><td className="p-4 text-center"><span className={`px-2 py-1 rounded text-xs font-bold ${exam.type === 'BRANS' ? 'bg-emerald-500/10 text-emerald-400' : `${colors.isDark ? 'bg-slate-800' : 'bg-slate-100'} ${colors.textSec}`}`}>{exam.type === 'BRANS' ? BRANCH_SUBJECTS.find(s=>s.id === exam.subject)?.label : exam.type}</span></td>{filterType === 'BRANS' && <td className="p-4 text-center text-xs">{exam.duration ? exam.duration + ' dk' : '-'}</td>}</>) : (EXAM_TYPES[filterType].groups.flatMap(g => g.sections).map(sec => (<td key={sec.key} className="p-4 text-center">{exam.details[sec.key]?.n.toFixed(1) || '-'}</td>)))}<td className="p-4 text-center font-bold text-orange-500 text-lg">{exam.totalNet.toFixed(2)}</td><td className="p-4 text-center">{!isStudent && <button onClick={(e) => requestDelete(e, exam.id)} className="p-2 hover:bg-red-500/20 text-slate-500 hover:text-red-500 rounded transition-colors group"><Trash2 size={16} className="group-hover:text-red-500" /></button>}</td></tr>))}{filteredExams.length === 0 && <tr><td colSpan="100%" className={`p-8 text-center ${colors.textSec}`}>Kayıt yok.</td></tr>}</tbody></table></div></Card></>)}
      {view === 'add' && (<div className="max-w-5xl mx-auto animate-fade-in"><Card title={examType === 'BRANS' ? 'Yeni Branş Denemesi' : 'Yeni Genel Deneme'}><div className="mb-8"><label className={`block text-xs font-medium ${colors.textSec} mb-2 uppercase tracking-wider`}>Sınav Türü</label><div className="flex flex-wrap gap-4">{[...Object.keys(EXAM_TYPES), 'BRANS'].map(type => (<button key={type} onClick={() => setExamType(type)} className={`flex-1 py-4 px-4 rounded-xl border-2 transition-all font-bold text-lg ${examType === type ? 'border-orange-500 bg-orange-500/10 text-orange-500' : `${colors.border} ${colors.inputBg} ${colors.textSec} hover:border-slate-500`}`}>{type === 'BRANS' ? 'Branş' : EXAM_TYPES[type].label}</button>))}</div></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">{examType === 'BRANS' && (<div><label className={`block text-xs font-medium ${colors.textSec} mb-2 uppercase tracking-wider`}>Branş Seçimi</label><select className={`w-full ${colors.inputBg} border ${colors.inputBorder} rounded-lg px-4 py-3 ${colors.text} text-sm focus:border-orange-500 outline-none`} value={formData.branchSubject} onChange={e => setFormData({...formData, branchSubject: e.target.value})}>{BRANCH_SUBJECTS.map(sub => <option key={sub.id} value={sub.id}>{sub.label}</option>)}</select></div>)}<Input label="Deneme Adı *" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Örn: 3D Matematik Branş-1" /><Input label="Tarih *" type="date" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} />{examType !== 'BRANS' && (<div><label className={`block text-xs font-medium ${colors.textSec} mb-2 uppercase tracking-wider`}>Zorluk</label><select className={`w-full ${colors.inputBg} border ${colors.inputBorder} rounded-lg px-4 py-3 ${colors.text} text-sm focus:border-orange-500 outline-none`} value={formData.difficulty} onChange={e => setFormData({...formData, difficulty: e.target.value})}><option>Kolay</option><option>Orta</option><option>Zor</option><option>Çok Zor</option></select></div>)}{examType === 'BRANS' && <Input label="Süre (dk) - İsteğe Bağlı" type="number" value={formData.duration} onChange={e => setFormData({...formData, duration: e.target.value})} placeholder="Örn: 45" />}</div><div className="space-y-6 mb-8"><h4 className={`text-orange-500 font-bold mb-4 border-b ${colors.border} pb-2 uppercase text-sm tracking-wider`}>Sonuç Girişi</h4>{examType === 'BRANS' ? (<div className={`${colors.bgCard} border ${colors.border} rounded-xl p-6 max-w-md`}><div className="flex items-center justify-between"><span className={`font-bold ${colors.text} text-lg`}>{BRANCH_SUBJECTS.find(s=>s.id === formData.branchSubject)?.label}</span><div className="flex gap-4"><div><label className="text-xs text-emerald-500 font-bold block mb-1">Doğru</label><input type="number" className={`w-20 ${colors.inputBg} border ${colors.inputBorder} rounded p-3 ${colors.text} text-center font-bold text-lg focus:border-emerald-500 outline-none`} value={formData.details[formData.branchSubject]?.d || ''} onChange={e => updateDetail(formData.branchSubject, 'd', e.target.value)} /></div><div><label className="text-xs text-red-500 font-bold block mb-1">Yanlış</label><input type="number" className={`w-20 ${colors.inputBg} border ${colors.inputBorder} rounded p-3 ${colors.text} text-center font-bold text-lg focus:border-red-500 outline-none`} value={formData.details[formData.branchSubject]?.y || ''} onChange={e => updateDetail(formData.branchSubject, 'y', e.target.value)} /></div></div></div></div>) : (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">{EXAM_TYPES[examType].groups.map((group, idx) => (<div key={idx} className={`${colors.bgCard} border ${colors.border} rounded-xl p-5`}><h5 className={`${colors.text} font-bold mb-4 flex items-center gap-2`}><div className="w-2 h-2 rounded-full bg-orange-500"></div>{group.title}</h5><div className="space-y-3">{group.sections.map(sec => (<div key={sec.key} className={`flex items-center justify-between ${colors.isDark ? 'bg-slate-800/50' : 'bg-slate-50'} p-3 rounded-lg border ${colors.border}`}><div><span className={`block text-sm font-medium ${colors.textSec}`}>{sec.label}</span><span className={`text-[10px] ${colors.textSec}`}>{sec.q} Soru</span></div><div className="flex gap-2"><input type="number" placeholder="D" className={`w-14 ${colors.inputBg} border ${colors.inputBorder} rounded p-2 ${colors.text} text-center text-sm focus:border-orange-500 outline-none`} value={formData.details[sec.key]?.d || ''} onChange={e => updateDetail(sec.key, 'd', e.target.value)} /><input type="number" placeholder="Y" className={`w-14 ${colors.inputBg} border ${colors.inputBorder} rounded p-2 ${colors.text} text-center text-sm focus:border-red-500 outline-none`} value={formData.details[sec.key]?.y || ''} onChange={e => updateDetail(sec.key, 'y', e.target.value)} /></div></div>))}</div></div>))}</div>)}</div>{examType !== 'BRANS' && (<div className={`mb-8 ${colors.bgCardTransparent} rounded-xl border ${colors.border} p-4`}><h4 className={`${colors.textSec} text-sm font-bold mb-4 flex items-center gap-2`}><Activity size="16" /> Sıralama Bilgileri (İsteğe Bağlı)</h4><div className="grid grid-cols-1 md:grid-cols-3 gap-6">{['turkey', 'city', 'school'].map(scope => (<div key={scope} className={`${colors.bgCard} p-4 rounded-lg border ${colors.border}`}><label className={`block text-xs font-bold ${colors.textSec} mb-2 uppercase`}>{scope === 'turkey' ? 'Türkiye' : scope === 'city' ? 'İl Geneli' : 'Kurum'}</label><div className="flex gap-2"><input placeholder="Sıralama" type="number" className={`w-full ${colors.inputBg} border ${colors.inputBorder} rounded p-2 text-sm ${colors.text}`} value={formData.stats[scope].rank} onChange={e => updateStat(scope, 'rank', e.target.value)} /><input placeholder="Katılım" type="number" className={`w-full ${colors.inputBg} border ${colors.inputBorder} rounded p-2 text-sm ${colors.text}`} value={formData.stats[scope].total} onChange={e => updateStat(scope, 'total', e.target.value)} /></div></div>))}</div></div>)}<div className={`flex justify-end gap-4 border-t ${colors.border} pt-6`}><Button variant="secondary" onClick={() => setView('list')}>İptal</Button><Button onClick={handleSave} size="large" icon={Save}>Kaydet</Button></div></Card></div>)}
      <DeleteConfirmModal isOpen={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} onConfirm={confirmDelete} />
    </div>
  );
};
...
const InterviewModule = ({ student, onUpdateStudent, user }) => {
  ...
  const [showNoteModal, setShowNoteModal] = useState(false); const [newNote, setNewNote] = useState({ note: "" });
  const [deleteConfirm, setDeleteConfirm] = useState(null);
  const colors = useThemeColors();

  const handleAddInterview = () => { if(!newInterview.note) return; const interview = { id: Date.now(), ...newInterview }; onUpdateStudent({ ...student, interviews: [interview, ...student.interviews] }); setShowModal(false); setNewInterview({ date: new Date().toISOString().split('T')[0], title: "", note: "" }); };
  const deleteInterview = (id) => { const filtered = student.interviews.filter(i => i.id !== id); onUpdateStudent({ ...student, interviews: filtered }); };
  const handleAddNote = () => { if(!newNote.note) return; const noteEntry = { id: Date.now(), date: new Date().toLocaleDateString('tr-TR'), text: newNote.note }; const updatedNotes = [noteEntry, ...(student.teacherNotes || [])]; onUpdateStudent({ ...student, teacherNotes: updatedNotes }); setShowNoteModal(false); setNewNote({ note: "" }); };
  ...
  return (
    <div className="animate-fade-in space-y-8">
      <div><div className="flex justify-between items-center mb-4"><h2 className={`text-2xl font-bold ${colors.text}`}>Görüşme Kayıtları</h2><Button onClick={() => setShowModal(true)} icon={Plus}>Yeni Görüşme</Button></div><div className="grid gap-4">{student.interviews.map(interview => (<div key={interview.id} className={`${colors.bgCard} border ${colors.border} rounded-xl p-6 relative group`}><div className="flex justify-between mb-2"><h3 className={`font-bold ${colors.text}`}>{interview.title}</h3><div className="flex items-center gap-3"><span className={`text-xs ${colors.textSec}`}>{interview.date}</span><button onClick={(e) => requestDelete(e, 'interview', interview.id)} className="text-slate-600 hover:text-red-500 p-2"><Trash2 size={16}/></button></div></div><p className={`${colors.textSec} text-sm whitespace-pre-wrap`}>{interview.note}</p></div>))}{student.interviews.length === 0 && <div className={`text-center ${colors.textSec} py-4 text-sm`}>Henüz görüşme kaydı yok.</div>}</div></div>
      <div className={`border-t ${colors.border} my-6`}></div>
      <div><div className="flex justify-between items-center mb-4"><h2 className={`text-2xl font-bold ${colors.text} flex items-center gap-2`}><MessageSquare className="text-orange-500"/> Öğrenciye Notlar</h2><Button onClick={() => setShowNoteModal(true)} variant="secondary" icon={Edit2}>Not Ekle</Button></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{(student.teacherNotes || []).map(note => (<div key={note.id} className={`${colors.bgCardTransparent} border ${colors.border} rounded-lg p-4 relative`}><div className="text-xs text-orange-400 font-bold mb-2">{note.date}</div><p className={`${colors.textSec} text-sm italic whitespace-pre-wrap`}>"{note.text}"</p><button onClick={(e) => requestDelete(e, 'note', note.id)} className="absolute top-2 right-2 text-slate-600 hover:text-red-500 p-1"><Trash2 size={14}/></button></div>))}{(!student.teacherNotes || student.teacherNotes.length === 0) && <div className={`col-span-full text-center ${colors.textSec} py-4 text-sm`}>Öğrenci paneline düşecek bir not eklemediniz.</div>}</div></div>
      {showModal && (<Modal title="Yeni Görüşme" onClose={() => setShowModal(false)}><div className="space-y-4"><Input label="Tarih" type="date" value={newInterview.date} onChange={e => setNewInterview({...newInterview, date: e.target.value})} /><Input label="Başlık" value={newInterview.title} onChange={e => setNewInterview({...newInterview, title: e.target.value})} /><textarea className={`w-full ${colors.inputBg} border ${colors.inputBorder} p-3 rounded ${colors.text} h-32 focus:border-orange-500 outline-none`} value={newInterview.note} onChange={e => setNewInterview({...newInterview, note: e.target.value})} placeholder="Görüşme notları..." /><Button onClick={handleAddInterview}>Kaydet</Button></div></Modal>)}
      {showNoteModal && (<Modal title="Öğrenci Paneline Not Ekle" onClose={() => setShowNoteModal(false)}><div className="space-y-4"><div className="bg-orange-500/10 p-3 rounded border border-orange-500/20 text-orange-400 text-xs">Buraya yazdığınız notlar öğrenci kendi paneline girdiğinde "Öğretmenden Notlar" bölümünde görünecektir.</div><textarea className={`w-full ${colors.inputBg} border ${colors.inputBorder} p-3 rounded ${colors.text} h-32 focus:border-orange-500 outline-none`} value={newNote.note} onChange={e => setNewNote({...newNote, note: e.target.value})} placeholder="Öğrenciye iletmek istediğiniz not..." /><Button onClick={handleAddNote}>Notu Yayınla</Button></div></Modal>)}
      <DeleteConfirmModal isOpen={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} onConfirm={confirmDelete} />
    </div>
  );
};
...
const LoginScreen = ({ onLogin, students, admins, isFirebase, isLoading }) => {
  ...
  const handleLogin = () => {
    setError('');
    if (activeTab === 'admin') {
      if (password === 'canmete21') { onLogin({ role: 'superadmin', id: SUPER_ADMIN_ID, name: 'Hamza Metehan Kılıç' }); return; }
      const adminUser = admins.find(a => a.username === username && a.password === password);
      if (adminUser) { onLogin({ role: 'admin', id: adminUser.id, name: adminUser.name }); return; }
...
const MainApp = () => {
  ...
  return (
    <div className={`flex flex-col md:flex-row h-screen ${colors.bgMain} ${colors.text} overflow-hidden font-sans transition-colors duration-300`}>
      ...
      <aside className={`fixed inset-y-0 left-0 w-64 ${colors.bgMain} border-r ${colors.border} flex flex-col z-50 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className={`p-6 border-b ${colors.border} flex flex-col gap-1`}><div className="flex items-center gap-2 text-orange-500 font-bold text-lg"><Users /><span>Hamza Metehan Kılıç</span></div><div className={`text-xs ${colors.textSec} ml-8`}>Uzman Psikolojik Danışman</div></div>
